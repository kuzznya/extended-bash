...
defined() {
  grep -xq "$1 .*" $TYPES && return 0 || return 255
}

# exists <instance>
# If instance exists then return 0 else return -1
exists() {
  grep -xq "$1 .*" $INSTANCES && return 0 || return 255
}

# propdefined <type> <prop>
# Check if prop exists in type
propdefined() {
  grep -xq "$1 $2" $TYPES && return 0 || return 255
}
...
propget() {
  local id=$1
  local prop=$2
  ! exists "$id" && instance_error "instance $id not found" && return 255
  ! propdefined "$(typeof "$id")" "$prop" && type_error "type does not have prop $prop" && return 255
...
propset() {
  ...
  cat $INSTANCE_PROPS | while read -r line ; do
